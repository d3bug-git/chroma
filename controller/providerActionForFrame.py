...
class ProviderActionForFrame(object):
    ...
    def getActionWhenQuit(self, frameName):
        ...
        method = getattr(self, method_name, lambda: print(
            "action_when_quit_not_found_for: "+frameName))
        return method()
...
# ********************************Action when quit********************************
    ...
    def action_when_quit_CONFIG_TIME(self):
        ...
        ChromaAnalyse.getInstance().setDuration(
            int(RootView.getInstance().getFrame().getTimeConfigured()))
        pub.sendMessage("DURATION_OF_ANALYSE",
                        duration=ChromaAnalyse.getInstance().getDuration())
        RootView.getInstance().unbind(
            "<<"+self.controller.convertBrocheToBrocheName(Broche.BUTTON_PLUS)+">>")
        RootView.getInstance().unbind(
            "<<"+self.controller.convertBrocheToBrocheName(Broche.BUTTON_MOINS)+">>")
    ...
    def action_when_quit_REAL_TIME_GRAPH(self):
        ...
        RootView.getInstance().bind("<<"+self.controller.convertBrocheToBrocheName(Broche.BUTTON_OK) +
...
# ********************************Action when go********************************
    ...
    def action_when_go_to_CONFIG_TIME(self, frame: ConfigTimeFrame):
        RootView.getInstance().bind("<<"+self.controller.convertBrocheToBrocheName(Broche.BUTTON_PLUS) +
                                    ">>", frame.incrementTimeConfigured)
        RootView.getInstance().bind("<<"+self.controller.convertBrocheToBrocheName(
            Broche.BUTTON_MOINS)+">>", frame.decrementTimeConfigured)
        return frame
